...
function AddUser() {
  ...
  const onUserCreate = async (newUser) => {
    ...
    try {
      let res = await fetch("https://user-management-app-t269.onrender.com/user-api/users", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="text-center">
      ...
      <form onSubmit={handleSubmit(onUserCreate)} className="max-w-96 mx-auto mt-10">
        ...
        />
        <button type="submit" className="text-2xl bg-blue-300 text-lime-50 px-8 py-4">
          Add User
        </button>
      </form>
    </div>
